export class Sudoku {
  ...
  resolverSudoku(indice = 0) {
    ...
    for (let num = 1; num <= 9; num++) {
      if (this.esSeguro(indice, num)) {
        this.originalBoard[indice] = num;

        if (this.resolverSudoku(indice + 1)) {
          return true;
        }

        this.originalBoard[indice] = 0;
      }
    }
  ...
  esSeguro(indice, numero) {
    ...
    for (let f = 0; f < 3; f++) {
      for (let c = 0; c < 3; c++) {
        if (
          this.originalBoard[(inicioFila + f) * 9 + (inicioCol + c)] === numero
        ) {
          return false;
        }
      }
  ...
  keydownEvent(e) {
    if (
      !this.keyDownNumbers.includes(e.key) &&
      !this.keyDownArrows.includes(e.key)
    ) {
      return;
    }
  ...
  endGame() {
    ...
    const sinErrores = !celdas.some((div) =>
      div.classList.contains("bg-red-200"),
    );
    console.log(sinErrores);

    const todoCorrecto = celdas.every(
      (div) => div.textContent === div.dataset.valorReal,
    );
